...
function parseArgs(argv) {
  ...
  const vitestArgs = separatorIndex >= 0 ? argv.slice(separatorIndex + 1) : [];
  let name = 'vitest';
  ...
  return {
    name: name.replace(/[^a-zA-Z0-9_-]+/g, '-').replace(/^-+|-+$/g, '') || 'vitest',
    vitestArgs: vitestArgs.length ? vitestArgs : ['run'],
  };
}
...
async function main() {
  const { name, vitestArgs } = parseArgs(process.argv.slice(2));
  const startedAt = new Date();
  const stamp = startedAt.toISOString().replace(/[:.]/g, '-');
  const runRoot = path.join(reportRoot, `${stamp}-${name}`);
  const logPath = path.join(runRoot, 'vitest.log');
  ...
  const summary = {
    name,
    command: ['vitest', ...vitestArgs].join(' '),
    exitCode,
  ...
  console.log(`\n[vitest-report] summary -> ${summaryPath}`);
  console.log(`[vitest-report] latest  -> ${latestSummaryPath}`);
