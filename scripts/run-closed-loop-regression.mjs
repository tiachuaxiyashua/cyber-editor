...
async function runScenario({ repoRoot, suiteArtifactRoot, scenario, resetRecent }) {
  ...
  fs.writeFileSync(
    ...
    [
      `# ${scenario.projectName} 闭环验证报告`,
      '',
      `- 运行时间：${new Date().toISOString()}`,
      `- 样例需求：${scenario.sampleIntent}`,
      `- 工程目录：${projectRoot}`,
      `- 导出包目录：${exportRoot}`,
      '- 说明：创建工程时的系统目录选择器仍通过 IPC 绕过；进入工程后的需求输入、阶段推进、最近工程重开和 OpenSpec 导出均通过真实界面完成。',
  ...
  assert.match(fs.readFileSync(path.join(exportedOpenSpecRoot, 'roadmap.md'), 'utf8'), /OpenSpec Roadmap/);
  assert.match(fs.readFileSync(path.join(exportedOpenSpecRoot, 'changes', changeName, 'proposal.md'), 'utf8'), /### Why/);
  assert.match(fs.readFileSync(path.join(exportedOpenSpecRoot, 'changes', changeName, 'tasks.md'), 'utf8'), /## 1\.\s+/);
  assert.ok(resolveDeliveryExportFile(exportRoot, 'delivery-package.md'));
  assert.ok(resolveDeliveryExportFile(exportRoot, 'delivery-package.txt'));
  assert.ok(resolveDeliveryExportFile(exportRoot, 'delivery-package.pdf'));
...
async function main() {
  ...
  const suiteArtifactRoot = path.resolve(repoRoot, 'artifacts', 'closed-loop-regression');
  ...
  console.log(`[closed-loop-regression] completed: ${path.join(suiteArtifactRoot, 'report.md')}`);
}

main().catch((error) => {
  console.error('[closed-loop-regression] failed');
  console.error(error);
  process.exitCode = 1;
});
